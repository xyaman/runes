...
// pub fn cursorGoTo(self: *Self, out: *std.io.Writer, x: usize, y: usize) !void {
//     if (y >= self.term_h) {
//         // calculate how many lines we need to scroll
//         const lines_to_scroll = y - self.term_h + 1;
//         try mibu.scroll.down(out, lines_to_scroll);
//
//         const term_w = self.term_w;
//         const term_h = self.term_h;
//
//         var row: usize = 0;
//         while (row < self.term_h - 1) : (row += 1) {
//             @memcpy(self.buffers[0][row * term_w .. (row + 1) * term_w], self.buffers[0][(row + lines_to_scroll) * term_w .. (row + lines_to_scroll + 1) * term_w]);
//             @memcpy(self.buffers[1][row * term_w .. (row + 1) * term_w], self.buffers[1][(row + lines_to_scroll) * term_w .. (row + lines_to_scroll + 1) * term_w]);
//         }
//
//         // clear the new bottom rows
//         row = term_h - lines_to_scroll;
//         while (row < term_h) : (row += 1) {
//             @memset(self.buffers[0][row * term_w .. (row + 1) * term_w], .{});
//             @memset(self.buffers[1][row * term_w .. (row + 1) * term_w], .{});
//         }
//
//     }
//
//     // Finally, move the terminal cursor
//     try mibu.cursor.goTo(out, x, y);
// }
...
pub fn engrave(self: *Self, out: *std.Io.Writer) !void {
    ...
    var last_draw_index: usize = 0;
    var y: usize = 0;

    // scroll if the next render will be out of screen
    if (self.cursor_y != 0 and self.render_h + self.cursor_y > self.term_h) {
        const lines_to_scroll = self.render_h + self.cursor_y - self.term_h;
        try mibu.scroll.up(out, lines_to_scroll);
        self.cursor_y -= lines_to_scroll;
    }

    while (y < self.term_h and y < self.render_h) : (y += 1) {
        try mibu.cursor.goTo(out, 1, self.cursor_y + y + 1);
        var x: usize = 0;
        while (x < self.term_w) : (x += 1) {
            ...
            if (!std.mem.eql(u8, buf[i].bytes(), prev[i].bytes()) or !buf[i].style.equals(prev[i].style)) {
                ...
                const prev_style: Rune.Style = if (last_draw_index > 0) buf[last_draw_index - 1].style else .{};
                try applyStyle(out, buf[i].style, prev_style);
                try out.print("{s}", .{buf[i].bytes()});
                last_draw_index = i;
            } else {
                try mibu.cursor.goTo(out, x + 2, self.cursor_y + y + 1);
            }
        }
    }
